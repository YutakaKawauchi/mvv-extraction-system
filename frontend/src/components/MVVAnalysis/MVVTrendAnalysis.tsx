...
export const MVVTrendAnalysis: React.FC = () => {
  ...
  const { keywordAnalysis, categoryTrends, loading } = useMemo(() => {
    ...
    // カテゴリ別トレンド分析
    const categories = [...new Set(validCompanies.map(c => c.category))];
    const categoryTrends: CategoryTrend[] = categories.map(category => {
      const categoryCompanies = validCompanies.filter(c => c.category === category);
      const categoryKeywords = keywordAnalysis.filter(kw => 
        kw.categories.includes(category)
      );
  ...
  const filteredKeywords = useMemo(() => {
    ...
    return filtered;
  }, [keywordAnalysis, selectedType, selectedCategory]);

  const categories = useMemo(() => {
    return ['all', ...categoryTrends.map(ct => ct.category)];
  }, [categoryTrends]);
