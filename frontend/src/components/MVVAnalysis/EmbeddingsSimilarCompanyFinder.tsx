/**
 * Embeddings-based Similar Company Finder
 * Uses real-time embeddings data from IndexedDB
 */
...
import type { CompanyWithScores } from '../../stores/embeddingsAnalysisStore';
...
const EmbeddingsSimilarCompanyFinder: React.FC = () => {
  const { 
  ...
  const handleCompanySelect = async (company: CompanyWithScores) => {
  ...
  const analyzeTextSimilarity = (company1: CompanyWithScores, company2: CompanyWithScores) => {
  ...
  const calculateEnhancedSimilarity = (company1: CompanyWithScores, company2: CompanyWithScores) => {
  ...
  // 類似度の理由を説明（改善版）
  const explainSimilarity = (company1: CompanyWithScores, company2: CompanyWithScores, similarity: number, textAnalysis: any) => {
  ...
  // マークダウン形式でMVV全体をコピー
  const copyAsMarkdown = async (company: CompanyWithScores) => {
    const parts = [];
  ...
  const MVVTooltip = ({ company, children }: { company: CompanyWithScores, children: React.ReactNode }) => {
...
export default EmbeddingsSimilarCompanyFinder;